...
/*
	switch(mode){
		...
		case 'R' :
			...
			printf("Starting Simulator in continous mode...\n\n");	
			printf("Execution (Register RO output:):\n");
			size=romFunction(ROM,argv[1]);
			pc=0;
			while(1){
		...
		case 'S':
			printf("Loading binary file: fibonacci.bin\nStarting Simulator in step-by-step mode...\n");
			size=romFunction(ROM,argv[1]);
			pc=0;
			while(1){
				instruction=fetch(ROM,pc);
				...
				temp=s;
				ALU(RA,RB,((temp>>2)&1)==1,&carry,&sum);
				//printf("inst is %d",ROM[pc]);
				//printf("value for enabo %d%d%d\n",enRA,enRB,enRO);
				if(enRA==1)
					RA=mux2_1(sum,s,sreg);
				if(enRB==1)
					RB=mux2_1(sum,s,sreg);
